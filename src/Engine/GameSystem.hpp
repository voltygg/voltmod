 ...
 * touched during Precache::Shutdown to detach the live game system.
 */
 ...
 * Precache so queued resource paths reach the session manifest.
 */
class PrecacheGameSystem final : public CBaseGameSystem
{
public:
    explicit PrecacheGameSystem(Precache& owner) : _owner(owner) {}
