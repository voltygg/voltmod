...
Result<uint64_t> ButtonPressMessage::ReadVarint(std::string_view bytes, size_t& at)
{
    uint64_t value = 0;
...
Status ButtonPressMessage::SkipField(std::string_view bytes, size_t& at, uint32_t wireType)
{
    switch (wireType)
    {
    case WireVarint:
