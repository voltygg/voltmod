...
`VoltMod::HL2SDK` and `VoltMod::Metamod` and includes the shipped
`cmake/VoltModPlugin.cmake` and `cmake/VoltModTests.cmake` build modules. Those
modules provide `voltmod_add_plugin` and `voltmod_add_tests`.
