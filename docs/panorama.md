...
A family is found by a plain scan for `.Prefix--variant`, layout order first and then stylesheet
...
| `bar` | `bar(id)` | a meter; pair with `bar.css.j2`'s `fill_rules(cls, steps)` for the `Step--0`..`Step--<steps>` width rules; `Hidden` on the bar takes it away |
| `toast` | `toast(id)` | a notice that fades in while class `Show` is on it |
| `icons` | `icons(id, set)` | one `<Image>` per PNG in the icon set, or per `(name, icon)` pair of client icons, stacked; pair with `icons.css.j2`'s `show_rules(set)` so each `Icon--<name>` class uncollapses its own image |
...
screen that includes it, which is what lets two screens share a row and still look different. A
static modifier uses one dash (`Button-ghost`); `--` reads as a server-written class family.
