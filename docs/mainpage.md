...
**CS2-Kit** is a C++23 library for building [Counter-Strike 2](https://www.counter-strike.net/) server plugins with [Metamod:Source 2.0](https://www.metamodsource.net/). You describe your plugin's behavior - commands, menus, effects, database rows - as data, and the kit handles the engine: SDK interface resolution, hooks, player tracking, message transport, signature scanning. Everything it owns lives on one @ref CS2Kit::Runtime, handed to your plugin for the duration of a load.

A working plugin is a `CommandSpec` and forty lines of skeleton. Generate one with `cs2kit new-plugin` and go from there (see @ref getting_started).

<h2>Modules</h2>

| Module | Namespace | What's in it |
|--------|-----------|--------------|
| **Core** | `CS2Kit::Core` | `PluginPolicy`, `Scheduler`, `SlotEvents`, `Subscription`, `EffectManager` and effect descriptors, logging, translations with `{token}` substitution, SteamID and duration parsing, chat colors, string/time helpers, angle math, decaying scores, config validation |
...
| **App** | `CS2Kit::App` | The composition root: `MetamodPlugin` (Metamod lifecycle), `JsonConfig`, `StatusService`, `ServiceExchange` for cross-plugin interfaces |
