...
| Menus | Panorama menus (custom HUD layouts) with a WASD center-HTML driver as the fallback, typed rows, pickers, and multi-step flows | Built-in builder with buttons, inputs, sliders, choices, toggles, and more | S2SDK exposes user-message and UI primitives; no equivalent high-level workflow is documented as part of the core stack | Chat and center-HTML menu APIs |
