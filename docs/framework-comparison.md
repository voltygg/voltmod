...
| Entities and schemas | `Entity`/`Pawn`/`Controller` value wrappers whose schema fields are typed members: each offset resolves once per process with a base-class walk, and a write to a networked field dirties it for replication on its own. Plus storable `EntityRef` handles, pawn operations, and transmit filters | Entity creation/query, generated schema types, handles, inputs, and outputs | Handle-based entities and schema get/set APIs through S2SDK | Generated schema classes, entity utilities, handles, and state-change helpers |
