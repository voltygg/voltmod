...
`cs2kit package <build|publish|prune|watch>`. All four used to live inside workflow
...
packages, the repo root for cs2-kit itself.
"""
...
CLOUDSMITH_OWNER, CLOUDSMITH_REPO = "voltygg", "cs2-kit"
...
def _build_kit(use_lockfile: bool) -> None:
    ...
    for postgres in ("False", "True"):
        _create(ROOT, extra, ["-o", f"cs2-kit/*:with_postgres={postgres}"])
...
def add_parser(subparsers) -> None:
    """Register `cs2kit package ...` on the top-level CLI."""
    ...
    for name, fn, help_text in (
        ...
        p.add_argument("--no-lockfile", action="store_true",
                       help="build cs2-kit without conan.lock")
