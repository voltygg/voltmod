...
- Game events go through `GameEvents::On<T>` with a struct in `Events/EventTypes.hpp`. No string form. The structs are generated by `voltmod framework eventgen` from the game's `.gameevents` files, never edited by hand (`bullet_impact` alone is hand-written in `Events/BulletImpact.hpp`). The player an event is about is `Slot`, and `On` never delivers an invalid one.
