"""Process and toolchain plumbing shared by every command: tools, MSVC, Conan remote, presets."""
...
# The one table both `voltmod build` and `voltmod doctor` judge a toolchain against, so doctor
# cannot report a toolchain the build then rejects.
...
def tool_version(tool: str) -> tuple[str, tuple[int, ...]]:
    """Return a tool's first `--version` line and the numbers parsed out of it.

    The numbers are empty when the banner carries none, which compares below every minimum.
    """
...
def require_build_tools() -> None:
    ...
    for tool, minimum in MINIMUM_VERSIONS.items():
        _, actual = tool_version(tool)
        if actual < minimum:
...
def conan_home() -> Path:
    """Conan's own home directory, honouring CONAN_HOME."""
    return Path(os.environ.get("CONAN_HOME", Path.home() / ".conan2"))


def profile_dirs(repo_root: Path) -> tuple[Path, ...]:
    """Where Conan profiles are looked for, the project's own first."""
    return (repo_root / "conan/profiles", conan_home() / "profiles")


def host_profile(repo_root: Path, preset: str) -> tuple[Path, list[str]]:
...
def ensure_msvc_env() -> None:
    ...
    out = subprocess.run(
        f'cmd /c "{vcvars}" >nul && set', check=True, text=True, capture_output=True
    ).stdout
