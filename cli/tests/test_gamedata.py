"""Cover signature matching with synthetic bytes; a wrong verdict can misbind an engine call."""

import pytest

from voltmod.errors import VoltmodError
from voltmod.gamedata import check_signatures, pattern_regex, replace_pattern
...
def test_a_pattern_that_still_matches_once_is_left_alone():
    ...
    results = check_signatures(gamedata("48 8B 01 02"), binaries, {})
...
def test_a_pattern_matching_twice_is_ambiguous_rather_than_repaired():
    ...
    results = check_signatures(gamedata("48 8B"), binaries, {})
...
def test_a_moved_displacement_is_wildcarded_and_named_after_its_schema_field():
    ...
    results = check_signatures(gamedata("3B 99 B0 04 00 00 7D 5E"), binaries, schema)
...
def test_two_viable_displacements_are_refused_rather_than_guessed_between():
    ...
    results = check_signatures(gamedata("05 00 00 00 99 06 00 00 00"), binaries, {})
...
def test_a_miss_no_displacement_explains_is_reported_not_repaired():
    ...
    results = check_signatures(gamedata("48 8B 01 02 03 04"), binaries, {})
...
def test_an_entry_for_the_other_platform_is_skipped():
    ...
    assert check_signatures(gamedata("48 8B"), binaries, {}) == []
...
def test_replacing_an_entry_the_file_does_not_have_is_refused():
    with pytest.raises(VoltmodError, match="no entry named Missing"):
        replace_pattern('{"signatures": {}}', "Missing", "48", "??")
