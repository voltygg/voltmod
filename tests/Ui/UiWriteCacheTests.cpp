...
TEST_CASE("The first write of a value is new, and repeating it is not")
{
    ...
    CHECK(cache.Update(0, "vm_row0_label", "text", "Kick"));
    CHECK_FALSE(cache.Update(0, "vm_row0_label", "text", "Kick"));
    CHECK(cache.Update(0, "vm_row0_label", "text", "Ban"));
    CHECK_FALSE(cache.Update(0, "vm_row0_label", "text", "Ban"));
}
...
TEST_CASE("Panels and variables are remembered apart")
{
    ...
    CHECK(cache.Update(0, "vm_row1_label", "text", "Kick"));
    CHECK(cache.Update(0, "vm_row0_label", "other", "Kick"));
}

TEST_CASE("A panel and a variable that concatenate the same way stay distinct")
{
    UiWriteCache cache;
    CHECK(cache.Update(0, "row", "0_label", "Kick"));
    CHECK(cache.Update(0, "row0", "_label", "Kick"));
    CHECK(cache.Update(0, "row0_", "label", "Kick"));
}

TEST_CASE("Slots are remembered apart")
{
    UiWriteCache cache;
    CHECK(cache.Update(0, "vm_title", "text", "Admin Panel"));
    CHECK(cache.Update(1, "vm_title", "text", "Admin Panel"));
    CHECK_FALSE(cache.Update(0, "vm_title", "text", "Admin Panel"));
}
...
{
    UiWriteCache cache;
    CHECK_FALSE(cache.Update(-1, "vm_title", "text", "Admin Panel"));
    CHECK_FALSE(cache.Update(VoltMod::MaxPlayers, "vm_title", "text", "Admin Panel"));
...
TEST_CASE("A slot changing hands forgets what its last occupant was told")
{
    ...
    CHECK(cache.Update(3, "vm_title", "text", "Admin Panel"));
    CHECK(cache.FirstFailure(3));
    CHECK(cache.Update(4, "vm_title", "text", "Admin Panel"));

    slots.Raise(3);

    CHECK(cache.Update(3, "vm_title", "text", "Admin Panel"));
    CHECK(cache.FirstFailure(3));
    CHECK_FALSE(cache.Update(4, "vm_title", "text", "Admin Panel"));
}
