...
// one embedded inline, which are exactly the two shapes SubObject and Inside exist to reach.
struct FakeServices
{
    int32_t Account = 0;
...
// FieldOffset remembers, so one that outlived a case would hold a pointer into the old cache.
#define SCHEMA_PTR_FIXTURE                                                 \
    ResetFieldCache();                                                     \
    SetFieldQuery(&FakeQuery);                                             \
    const FieldOffset kHealth{"Entity", "m_iHealth", sizeof(int32_t)};     \
    const FieldOffset kMoney{"Entity", "m_pMoney", sizeof(void*)};         \
    const FieldOffset kState{"Entity", "m_state"};                         \
    const FieldOffset kAccount{"Services", "m_iAccount", sizeof(int32_t)}; \
    const FieldOffset kCounts{"Services", "m_nCounts"};                    \
    const FieldOffset kEnabled{"Embedded", "m_bEnabled", sizeof(uint8_t)}; \
    const FieldOffset kMissing{"Entity", "m_notAField"}
...
TEST_CASE("A null SchemaPtr reads a fallback and refuses to write")
{
    ...
    CHECK(none.Get<int32_t>(kHealth) == 0);
    CHECK(none.Get<int32_t>(kHealth, -1) == -1);
    CHECK(none.Ptr<int32_t>(kHealth) == nullptr);
    CHECK_FALSE(none.Set<int32_t>(kHealth, 100));
...
TEST_CASE("An unresolved field reads the fallback rather than whatever sits at offset zero")
{
    ...
    CHECK(live.Get<int32_t>(kMissing, -1) == -1);
    CHECK_FALSE(live.Set<int32_t>(kMissing, 7));
...
TEST_CASE("Get and Set reach a field on the object itself")
{
    ...
    CHECK(live.Get<int32_t>(kHealth) == 42);
    CHECK(live.Set<int32_t>(kHealth, 100));
    CHECK(entity.Health == 100);

    ResetFieldCache();
}

TEST_CASE("SubObject follows a pointer field and null-checks the whole chain")
{
    SCHEMA_PTR_FIXTURE;
    ...
    CHECK(live.SubObject(kMoney).Get<int32_t>(kAccount) == 16000);
    CHECK(live.SubObject(kMoney).Set<int32_t>(kAccount, 800));
    CHECK(services.Account == 800);

    // A null pointer field ends the chain rather than dereferencing it.
    entity.Money = nullptr;
    CHECK_FALSE(static_cast<bool>(live.SubObject(kMoney)));
    CHECK(live.SubObject(kMoney).Get<int32_t>(kAccount, -1) == -1);

    // ...and so does an unresolved one, without ever reading the object.
    CHECK_FALSE(static_cast<bool>(live.SubObject(kMissing)));

    ResetFieldCache();
}

TEST_CASE("Inside reaches an embedded sub-object without dereferencing it")
{
    SCHEMA_PTR_FIXTURE;
    ...
    CHECK(live.Inside(kState).Get<uint8_t>(kEnabled) == 1);
    CHECK(live.Inside(kState).Set<uint8_t>(kEnabled, 0));
...
TEST_CASE("Ptr hands back the field itself so an array field can be indexed")
{
    ...
    int32_t* counts = live.SubObject(kMoney).Ptr<int32_t>(kCounts);
...
TEST_CASE("Two SchemaPtrs compare equal when they name the same object")
{
    ...
    CHECK(live.SubObject(kMoney) == SchemaPtr{&services});
